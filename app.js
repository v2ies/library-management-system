...
const LOAN_DAYS = 14;
const THEME_KEY = 'uphsd_theme';

/* ---------- Dark mode (local only, flat colors) ---------- */
...
function initTheme() {
  ...
  if (t !== 'dark' && t !== 'light') {
    t = document.documentElement.getAttribute('data-theme') === 'dark' ? 'dark' : 'light';
    try { localStorage.setItem(THEME_KEY, t); } catch (e) {}
  } else {
    if (t === 'dark') {
      document.documentElement.setAttribute('data-theme', 'dark');
...
const STORE_KEY = 'uphsd_library_v3'; // bumped: v3 clears mock students/records
...
function getCategories() {
  const used = books().map(b => b.category);
  const all = [...new Set([...DEFAULT_CATEGORIES, ...(DB.customCategories || []), ...used])];
  return all;
}
...
function updateCatDeleteBtn() {
  /* no-op: category delete moved to Manage Categories modal */
}
...
function refreshCategorySelects() {
  ...
  const makeOptsNoAll = (sel) => {
    const cur = sel.value;
    sel.innerHTML = cats.map(c => `<option>${esc(c)}</option>`).join('');
    if (cats.includes(cur)) sel.value = cur;
  };
  const bookFilter = document.getElementById('bookCatFilter');
  const stuFilter = document.getElementById('stuCatFilter');
  const bCat = document.getElementById('bCategory');
  if (bookFilter) makeOpts(bookFilter);
  if (stuFilter) makeOpts(stuFilter);
  if (bCat) makeOptsNoAll(bCat);
}

/* ---------- Session (persisted so login survives index <-> admin navigation) ---------- */
...
function flashToast(title, desc) {
  try { sessionStorage.setItem(FLASH_KEY, JSON.stringify({ title, desc })); } catch (e) {}
}
...
function refreshOverdue() {
  ...
  DB.records.forEach(r => {
    if (r.status === 'borrowed' && r.dueDate && r.dueDate < today) r.status = 'overdue';
  });
  save();
}
...
function toast(title, desc, err) {
  ...
  wrap.appendChild(el);
  setTimeout(() => { el.style.animation = 'slideOut .22s ease forwards'; setTimeout(() => el.remove(), 220); }, 3000);
}
...
// Renders a student's avatar: their uploaded photo if set, otherwise their name initial.
...
function showScreen(id) {
  ['screen-landing', 'screen-admin-landing', 'screen-admin-login', 'screen-student-gate', 'screen-admin', 'screen-student']
    .forEach(s => { const el = document.getElementById(s); if (el) el.classList.add('hidden'); });
  const target = document.getElementById(id);
  if (!target) {
    // Target lives on the other page — navigate there.
  ...
  window.scrollTo(0, 0);
  // Floating toggle is for landing/login only — admin/student headers have their own.
...
function renderStudentPortal() {
  ...
  const hn = document.getElementById('stuHeaderName');
...
let pendingPhotoData = undefined; // undefined = no change, null = removed, string = new dataURL
...
function renderStuSettings() {
  ...
  const rm = document.getElementById('settingsRemovePhotoBtn');
  if (rm) rm.classList.toggle('hidden', !s.photo);
  const e = document.getElementById('settingsErr');
  if (e) e.classList.add('hidden');
}
...
function handlePhotoChange(input) {
  ...
  reader.onload = () => {
    ...
    document.getElementById('settingsAvatar').outerHTML = avatarHTML(s.name, pendingPhotoData, 72, 26).replace('class="avatar"', 'class="avatar" id="settingsAvatar"');
...
function renderStuBrowse() {
  const searchEl = document.getElementById('stuBookSearch');
  const catEl = document.getElementById('stuCatFilter');
  const grid0 = document.getElementById('stuBooksGrid');
  ...
  const filtered = books().filter(b => {
    const mc = !cat || b.category === cat;
    const ms = !search || [b.title, b.author].some(f => f.toLowerCase().includes(search));
    return mc && ms;
  });
...
function requestBook(bId) {
  ...
  const my = records().filter(r => r.studentId === session.studentId);
  if (my.some(r => r.bookId === bId && (r.status === 'pending' || r.status === 'borrowed' || r.status === 'overdue'))) {
    toast('Already requested', 'You already have this book or a pending request.', true); return;
  }
...
function renderStuMyBooks() {
  const el = document.getElementById('stuMyBooks');
  if (!el) return;
...
/* ---------- Cross-tab sync (index.html <-> admin.html share localStorage) ---------- */
...
/* ---------- Page-aware boot (index.html vs admin.html share this file) ---------- */
function boot() {
  refreshOverdue();
  initTheme();
  refreshCategorySelects();
  showFlashToast();
  // file:// gives each page isolated storage — warn so cross-page data isn't a mystery.
  try {
  ...
  const isAdminPage = !!document.getElementById('screen-admin');
  const isIndexPage = !!document.getElementById('screen-landing');
  ...
  if (isIndexPage) {
    if (session.role === 'student' && studentById(session.studentId)) {
      // Restore student session after navigation/refresh (no welcome toast).
      showScreen('screen-student');
      switchStuTab('browse', document.querySelector('#screen-student .tab-btn'));
      renderStudentPortal();
    } else {
